...
def _image_metadata(path: Path, *, page_field: str) -> tuple[int, int]:
    try:
        ...
        raise PageXmlImportError(f"{page_field}.imageFilename is not a readable image: {path}") from error
...
def _page_payload(
    ...
    *,
    ...
    if declared_width is not None and declared_width != actual_width:
        raise PageXmlImportError(
            f"{page_field}.imageWidth={declared_width} does not match source image width {actual_width}"
        )
    if declared_height is not None and declared_height != actual_height:
        raise PageXmlImportError(
            f"{page_field}.imageHeight={declared_height} does not match source image height {actual_height}"
        )
    ...
    def visit(element: ET.Element, active_region_id: str | None) -> None:
        ...
        if local == "TextRegion":
            ...
            if region_id in regions:
                raise PageXmlImportError(f"{page_field} contains duplicate TextRegion id {region_id!r}")
