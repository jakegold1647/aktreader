# CONSENSUS MERGE — wave 004: Serock 1890 deaths, acts 41–49 (+ SkZ index, merged separately in the CLOSURE AUDIT)
Merged 28 Jul 2026 by the coordinating session (consensus stage — blindness no longer applies).
Reader A = claude subscription session, prompt v1.3.0 (97dfa6a7…), frozen 0eddf76.
Reader B = gpt-5.6 sol-ultra, prompt v1.3.0 (same hash), frozen d3c94a2. Per-line label sha256s are
recorded in the project's internal coordination log, which is not published in this repository.
Artifacts: wave003\Serock_1890_deaths_39-42.jpg (acts 41–42) + wave004\43-46 / 47-49 spreads.

**Prompt-bias check:** zero manufactured dual dates by either reader (fourth consecutive clean wave).
**Protocol-compliance finding (header-level, feeds lessons):** Reader B's ILLEGIBLE notes state
"2× quadrant and 3× line inspection" — BELOW the v1.3 rule's own 4–8× floor. Reader A worked at
4–9× throughout. B declared the full identity block ILLEGIBLE in acts 41, 42(partial), 49 and
omitted declarants/officiant/hours in most acts. This repeats and sharpens the wave-003
ILLEGIBLE-divergence phenomenon: the v1.3 rule was adopted but not executed at the prescribed
magnification. Not retroactive to these frozen labels; wave-005 enforcement note filed.

## Field-level merge (compact; AGREE entries CONFIDENT-eligible unless noted)

### ACT 41
| Field | Reader A | Reader B | CONSENSUS |
|---|---|---|---|
| reg. date | [unclear: 3/23] Sept, 7 AM | 30 Sept, no hour | 🔴 #4 pooled [3/23/30 Sept]; hour single-coverage |
| principal | Rafal Zylbersztejn, M, 4 y (closing-repeat check passed) | ILLEGIBLE (name/age), sex [unclear] | 🔴 #22 single-coverage → verification read; A's reading unopposed but unconfirmed |
| filiation | Moszek + Basia, traders | ILLEGIBLE | #22 (same verification read) |
| declarants | Majer Mendzylewski shopkeeper 25; Berek [unclear: Kochin/Kolpin] laborer 65 | (not recorded) | single-coverage; #22 |

### ACT 42
| reg. date | 6 Oct, 8 PM | 3 Oct | 🔴 #5 [шестаго/третьяго] |
| principal | [unclear: Ryfka/Rywka] **Ochberg**, F, ½ y | [unclear: Rejzla **Oren**?] | 🔴 #6 — A has closing-repeat support; B single shaky candidate |
| age | ½ year | ILLEGIBLE | #23 verification |
| filiation | Szmul + [Bejla Frajda?] née [Szelko-/Wielko-]broda | ILLEGIBLE | 🔴 #23 single-coverage verification |
| sex | female (умерла) | female | **AGREE** |

### ACT 43 — the Goldsztejn act (lexicon-relevant: family-of-interest surname; gold birth act 1/1890 is Symcha Aron Goldsztejn — DIFFERENT parents (this: Abram + C.), household linkage NOT asserted, corpus observation only)
| reg. 9 Oct / event 8 Oct (explicit day-word, both) | agree | agree | **AGREE** |
| principal | unnamed female infant, безъ имени | same, STATED_UNKNOWN | **AGREE — typed-state convergence (STATED_UNKNOWN)** |
| age | **6 days** («шесть дней») | **3 months** («три мѣсяца») | 🔴 #7 — not glyph-adjacent; one reader wrong |
| father | Abram | Abram (Аврамъ) | **AGREE** |
| mother | **Cyrka** (Цюрки) | **Czerna** (Черны) | 🔴 #8 |
| family surname | Гольдштейнъ (Goldfarb-trap checked, -штейнъ confirmed) | Гольдштейнъ | **AGREE → CONFIDENT-eligible** |

### ACT 44
| reg. date | 11 Oct, 1 PM | 12 Oct | 🔴 #9 [одиннадцатаго/двенадцатаго] |
| principal | **Moszek** Wajngrod, M, 4 y (closing check) | **Mejer** Wajngrod, M, 4 y | surname+age+sex **AGREE**; given 🔴 #10 [Мошекъ/Мейеръ] |
| father | **Srul** | **Hersz/Gerc** (Герца) | 🔴 #11 — not glyph-adjacent |
| mother | Ruchla | Ruchla | **AGREE** |
| mother née | **STATED_UNKNOWN** («урожденной неизвѣстно», ink read at 4×) | ILLEGIBLE | 🔴 #12 — typed-state arbitration: PRESENT-ink claim vs illegibility claim |
| declarant 1 | Abram **Wajngrod**, szkolnik, **65** | Awram **Wajngrub**, шинкарь (tavern-keeper), **67** | 🔴 #13 (three sub-splits) |
| declarant 2 | Mortka Wajngrod, trader, 33 | same | **AGREE** |

### ACT 45 — 🔴🔴🔴 IDENTITY FORK (wave headline #1)
| | Reader A | Reader B |
|---|---|---|
| verb/sex | умеръ, MALE | умерла, FEMALE |
| principal | **Josek Makowski, baker, 71** (closing repeat «Іоска Маковскаго») | **[unclear: Basia Konowska?], 47** (single shaky) |
| parents | late Michel? + Bejla | Michel + Bejla — **PARENTS AGREE ACROSS THE FORK** |
| survivors | widow [Chana?] née [Sudeknica/Sudeknita] | husband Mejer |
| reg. date | [27/28] Nov, 9 AM | 20 Oct |
| declarants | Josek Rubinsztejn smith 36 + Icek Rubinsztejn locksmith 50 | (not recorded) |
🔴 #1 (identity) + #2 (date). Note for arbitration: BOTH SkZ index transcriptions give a MALE
given name at act 45 (A: Маковскій Іосекъ; B himself: Миновскій **Юсекъ**) — Reader B's own
index read contradicts his act read. Candidates pooled neutrally regardless.

### ACT 46 — 🔴🔴🔴 IDENTITY FORK (wave headline #2)
| | Reader A | Reader B |
|---|---|---|
| principal | **Rojza Bornsztejn, F, 1 y** (closing repeat check) | **Ruchla Grosman, F, 2 y** |
| filiation | Jankel + Szyfra née Polkowicz | Izrael + Szajndla née Wolberg |
| reg. date | 28 Nov (hours BLANK, twice) | 28 Nov — **DATE AGREES** |
| declarants | Mordka Kanarek carter 31; Szymel [Ickowicz/Cukowicz] carter 38 | (not recorded) |
🔴 #3 — everything but the date forks. Two complete, internally-coherent readings of the same
pixels; the sharpest fork yet because neither is a fragment-phantom: one is wholesale.

### ACT 47
| reg. date | 11 Dec, noon | 10 Dec | 🔴 #14a |
| event | yesterday, 10 PM | **2 Dec** («второго числа сего мѣсяца»), 10 PM | 🔴 #14b — structural: relative vs explicit day-word |
| principal | Josek [unclear: Zalcberg/Zalberg], ½ y | Jusek **Zalcberg** (ц read) | surname **CONVERGES Зальцбергъ** (B's ц + A's index row); given AGREE |
| age | **½ year** | **[unclear: 40-something years?]** | 🔴 #15 — infant vs adult; massive split |
| father | [unclear: Madech/Manes] | Majer | 🔴 #16 pooled [Мадехъ/Манесъ/Майеръ] |
| mother | [La-Gita? compound, line-break join] née **Bogdan** | Laja, née ILLEGIBLE | #16 given pooled; née Bogdan single-coverage → verify |
| declarant 2 | Berek [unclear: Kaszu/Koltun] laborer 65 | Berek **Kiltur** laborer | → STANDING #20 |

### ACT 48
| reg. date | 20 Dec, 4 PM | 19 Dec | 🔴 #17 |
| principal | Dawid **Lejbka** [Szejngolc/Szejngolbur] , M, 3 wk | Dawid **Szejngolc** (ц read), M, 3 wk | surname **CONVERGES Шейнгольцъ**; middle name 🔴 #18a [present Лейбка / absent] |
| father | [unclear: Delusz/Dowusz] | **Lejbka** | 🔴 #18b — SLOT-RISK flagged: B's "father Lejbka" equals A's principal middle name; possible slot transposition; candidates pooled [Делушъ/Довушъ/Лейбка] |
| mother | Chaja Dwojra née **Szperling** | Chaja-Dwojra née **Szperling** | **AGREE → CONFIDENT-eligible** |
| declarant 1 | Lejba [Sztamger/Szmanger/Sztamberg] trader 65 | [unclear: Mota Sztam?] trader | 🔴 #19 pooled |
| declarant 2 | Berek **Koltun** (л-т-у-н distinct, per A) 65 | Berek **Kiltur** | STANDING #20 |

### ACT 49 — second red-ink rectification act (compare act 40)
| reg. 28 Dec 11 AM / event 27 Dec 5 PM / female | agree | agree | **AGREE** |
| black layer | Sura **Rozenek**, 1 week; Jankel + Ryfka née **Jedwabnik** (struck but legible) | ILLEGIBLE | 🔴 #21 single-coverage → verification read |
| red layer | [Sura/Sara] [Geldfisz/Geldblum], 3 months; father → Mortka; mother → [Frejda?] née [Butel/Bungel] | marginalia PRESENT (red corrections seen), content ILLEGIBLE | #21 |
| red margin attestation | «Исправленному … вѣрить» + signature — SAME formula class as act 40's attestation | (not transcribed) | #21; correction-protocol pattern now ×2 |
| declarants | Berek Koltun 65; Lejbka Kendzior shoemaker 54 | Berek Kiltur; [Lejb …?] | STANDING #20; Kendzior partially corroborated |
Layer note: BOTH SkZ transcriptions index act 49 under **Розенекъ** (the black layer) — see
closure audit: the index predates the rectification.

### STANDING ITEMS
- 🔴 **#20 — the recurring ~65-year-old witness (acts 47/48/49 + wave-003 acts):** candidates now
  [Колтунъ (A, ink-argued) / Килтуръ (B ×3) / Ковшунъ (wave-003 verification reader)] +
  occupation [работникъ / кладбищный служитель]. Merged with the REOPENED wave-003 item 20.
  This person's fields remain EXCLUDED from silver corpus-wide. Expert review, first chair.
- 🔴 **#24 — officiant, all nine acts:** A [unclear: Пшибышевскій?] ×9; B did not record the
  field. Wave-003 margin attestation was read «Пржибыш[евъ]». Pool once: [Пшибышевскій/
  Пржибышевскій/Пржибышевъ]. One verdict covers the whole register year.

## Disposition
- **CONFIDENT-eligible convergences:** act 43 (reg+event dates, STATED_UNKNOWN name, father,
  surname Гольдштейнъ); act 44 (surname, age, sex, mother); act 47 surname Зальцбергъ; act 48
  (surname Шейнгольцъ, age, sex, mother+née Szperling); act 49 (dates, sex); act 42 sex; act 46
  reg date.
- **24 numbered items** → arbitration/verification, priority: #1/#2 (act-45 fork), #3 (act-46
  fork), #21 (act-49 layers), #22 (act-41 single-coverage), #7 (Goldsztejn age), then dates
  (#4,5,9,14,17), names (#6,8,10,11,15,16,18,19,23), standing (#20,#24), typed-state (#12), #13.
- Single-coverage fields (B ILLEGIBLE / unrecorded) enter NOTHING until a fresh verification
  read supplies a second vote — wave-003 precedent.
- All promotions await Reader C 2-of-3; NO RESOLVED appendix yet.
- Acts 45 and 46 pre-nominated to the human gold sample if arbitration confirms either reading
  (identity-level forks, standing rule).

---

## WAVE 004 RESOLVED — Reader C arbitration applied (2-of-3 rule, 28 Jul 2026)
Reader C: fresh blind session, 4–8× upscaled crops of the original pixels; full report at
readerC_arbitration_wave004.md. 25 items: 20 disputes, 3 verification reads (single-coverage
fields), 1 standing officiant item, 1 index spot-check.

| # | Dispute | Verdict (C's certainty) | Promoted value |
|---|---|---|---|
| 1 | Act 45 IDENTITY FORK | «умеръ Іосекъ Маковскій», M, 71, пекарь; closing «Іоска Маковскаго» (SURE) | **Josek Makowski, baker, 71, son of the late Michel + Bejla** (parents agreed across the fork); "leaves widow" confirmed; widow's name/née stay single-coverage, NOT promoted |
| 2 | Act 45 reg. date | «двадцать седьмаго Ноября» (SURE) | **27 Nov 1890** |
| 3 | Act 46 IDENTITY FORK | «Ройза Борнштейнъ», 1 y, d. Янкеля + Шифры ур. Полковичъ (SURE) | **Rojza Bornsztejn, 1 y, Jankel + Szyfra née Polkowicz**; reg. 28 Nov was already agreed |
| 4 | Act 41 reg. date | «двадцать третьяго Сентября» (LEANING) | [PROBABLE: 23 Sept] |
| 5 | Act 42 reg. date | «шестаго Октября» (SURE) | **6 Oct** |
| 6 | Act 42 principal | «Рыфка Охбергъ» + index row (SURE) | **Ryfka Ochberg** |
| 7 | Act 43 age | «шесть дней» (SURE) | **6 days** (the Goldsztejn infant — act now fully resolved) |
| 8 | Act 43 mother | «Цюрка» (SURE) | **Cyrka** |
| 9 | Act 44 reg. date | «одиннадцатаго» (SURE) | **11 Oct** |
| 10 | Act 44 given | «Мошекъ» (SURE) | **Moszek Wajngrod** |
| 11 | Act 44 father | «Сруль» (SURE) | **Srul** |
| 12 | Act 44 née slot | ink physically reads «урожденной неизвѣстно.» complete (SURE) | **STATED_UNKNOWN stands** — typed-state dispute resolved for the PRESENT-ink claim |
| 13 | Act 44 declarant 1 | «Вайнгродъ, школьникъ, 65» (SURE) | **Abram Wajngrod, szkolnik, 65** |
| 14 | Act 47 dates | reg «одиннадцатаго Декабря»; event «вчерашняго числа» — the «двѣнадцать» is the HOUR (SURE) | **reg. 11 Dec; event 10 Dec** (B's explicit-day parse was the hour word) |
| 15 | Act 47 age | «полъ года», no «сорока…» present (SURE) | **½ year — infant** |
| 16 | Act 47 parents | father «Мадехъ» (LEANING); mother = line-break compound «Лаи-/Гити» (SURE) | father [PROBABLE: Madech]; mother **Laja-Gita (compound)**; née Bogdan remains single-coverage, NOT promoted |
| 17 | Act 48 reg. date | «двадцатаго Декабря» (SURE) | **20 Dec** |
| 18 | Act 48 names | «Лейбка» IS the principal's middle name; father «Делушъ» (ink «Делюша») (SURE/LEANING) | **Dawid Lejbka Szejngolc**; father [PROBABLE: Delusz]; no slot transposition; age NB «три недѣли» |
| 19 | Act 48 declarant 1 | NEITHER — C reads «Липа Штатингеръ»; index family «Штатынгеръ» | NEW [unclear: Штатингеръ/Штатынгеръ/Штамгеръ/Шмангеръ/Штамбергъ/Мота Штамъ] — no promotion; lexicon cross-ref: Штатзингеръ family (wave-003 acts 31/32) |
| 20 | STANDING witness | «Колтунъ» in all 7 acts checked (39,40,41,42,47,48,49); «работникъ» in every wave-004 act; «кладбищный служитель» verbatim once, act 39 (SURE) | **NOT CLOSED.** C's 7-act evidence recorded as strong support for Колтунъ/работникъ, with act-39 occupation as genuine per-act variance — but the wave-003 verification reader's «Ковшунъ» in acts 36–38 remains unexplained. Item stays on the EXPERT-REVIEW list, first chair; fields remain excluded from silver corpus-wide |
| 21 | Act 49 layers (verification) | Black: «Сура Розенекъ», «одну недѣлю», Янкеля + Рыфки ур. Едвабникъ (clear). Red: age «три мѣсяца» (clear), father «Мотки» (clear), surname «Гольдблюмъ» (LEANING), red given ≈ guess, red mother unresolved | **Black layer promoted in full: Sura Rozenek, 1 week, Jankel + Ryfka née Jedwabnik.** Red layer: age **3 months** + father **Mortka** promoted; surname [PROBABLE: Гольдблюмъ]; red mother stays [unclear]. **Act 49 → human/expert sample** (second rectification act; higher-resolution rescan recommended) |
| 22 | Act 41 (verification) | «умеръ Рафалъ Зильберштейнъ», ~4 y (age LEANING), s. Мошка и Баси Зильберштейновъ, traders; declarants Маеръ Мендзилевскій лавочникъ 25 + Берекъ Колтунъ работникъ 65 (SURE) | **Rafal Zylbersztejn, son of Moszek + Basia (traders); declarant Majer Mendzylewski, shopkeeper, 25** — second vote supplied; age [PROBABLE: 4 y]. Witness token feeds #20 (not promoted) |
| 23 | Act 42 filiation (verification) | age «полъ года» (clear); father «Манеса(?)» GUESS; mother «Блюмы Двойры(?)» LEANING; maiden «Шехтманъ(?)» GUESS — severe bleed-through | age **½ year** promoted (second vote). **Filiation UNRESOLVABLE at current scan quality** — all candidates pooled ([Szmul/Manes]; [Bejla Frajda/Bluma Dwojra]; née [Szelkobroda/Wielkobroda/Szechtman]), NOTHING promoted; **rescan queue** with act 49's red interlines |
| 24 | Officiant (standing, all 9 acts) | «Пржибышевскій» — abbreviated scrawl, post-П cluster fits «рж» (LEANING) | [PROBABLE: Пржибышевскій] for the register year — sub-gold, one verdict covers all acts |
| 25 | Index spot-check (10 lines) | as-written: 34 «Мяри Байля», 39 «Маковская Рухля», 40 «Бобекъ Гутманъ», 43 «Гольдш?тейнъ» (no given), 45 «Маковскій Іосекъ», 46 «Борнштейнъ Ройза», 47 «Зальцбергъ Іосекъ», 48 «Шейнгольцъ Давидъ Ле(й)бка», 49 «Розенекъ Сура» (SURE as-written) | Pre-rectification thesis CONFIRMED (40 and 49 both indexed under erroneous original names). Index row 34 «Мяри» independently corroborates the wave-003 Miara verdict; row 45 confirms the male reading |

**Scoreboard (n=18 attributable disputes, not a leaderboard):** A prevailed 18/18 (#1–18, with
#4 and #16-father inside A's own [unclear] pools); B 0; #19 resolved NEITHER (new candidate);
#20 held open by coordinator ruling despite C's 7-act evidence; #24 resolved from the pooled
candidates. Verification reads corroborated Reader A's single-coverage readings in acts 41 and
49 (black layer) and the act-42 age; act-42 filiation is genuinely unresolvable at this scan
quality. Reader B's misses concentrate in two classes: two wholesale identity forks (acts 45,
46) and systematic ILLEGIBLE over-declaration (see lessons).

### THE LESSONS OF WAVE 004
1. **Phantom-identity catches #3 and #4.** Act 45: Reader B produced «Бася Коновская, F, 47»
   where the ink reads a 71-year-old male baker — sex, name, age, and date all forked from one
   умеръ/умерла misparse plus a date slip; B's OWN index transcription («Миновскій Юсекъ»,
   male) contradicted his act read — an internal-consistency check between a reader's act and
   index outputs is a cheap new validator candidate. Act 46: B produced a COMPLETE, internally
   coherent alternative reading («Рухля Гросманъ», different parents, different née) — the
   sharpest fork yet because it is not a fragment-phantom; only the agreed date survived.
   Cross-vendor consensus caught both before gold, as designed.
2. **The v1.3 rule was adopted but not executed.** B's ILLEGIBLE notes cite 2–3× inspection
   against the rule's 4–8× floor; every field B declared ILLEGIBLE that C or the verification
   pass inspected at 4–8× was readable except act-42's filiation and act-49's red interlines.
   Wave-005 enforcement: notes must state ≥4× or the label fails ingest validation (mechanical
   check recommended to the builder).
3. **The pre-rectification index thesis is confirmed from the pixels** (item 25): the skorowidz
   carries the erroneous original names for BOTH corrected acts (40, 49). The index is a
   snapshot of the register before the corrections were entered and can never arbitrate
   red-vs-black questions.
4. **Item 20 discipline held:** even a 7-act SURE verdict does not close a cross-wave conflict
   with an unexplained contrary fresh read. Expert review, first chair, fields still out of
   silver.
5. **Rescan queue (genuine scan-quality limits, not reader failure):** act 42 filiation
   (bleed-through), act 49 red interlines, act 40's second red margin note in a different hand
   (no-vote observation).

### Disposition
- All promotions above are consensus-PROBABLE (2-of-3). Silver ingest opens when the
  coordinator posts the green-light.
- Human/expert sample additions: **acts 45, 46 (resolved identity forks, standing rule) and 49
  (rectification act)** — joining acts 6, 34, 39, 40-surname, and the item-20 witness.
- Wave 005 (acts 7–29) is GO — artifacts staged and hashed; closes the book.
